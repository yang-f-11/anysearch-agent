...
这是一个偏工程验证性质的项目，目前仍有一些可以继续改进的地方：

- 搜索和回答缓存主要保存在进程内存中，服务重启后不会持久化；
- 可用模型列表目前通过代码配置，后续可以改为从 `llm_hub` 动态发现；
- Search Decision 和 Query Rewrite 仍依赖 Prompt，存在误判可能；
- 当前没有完整的自动化评测集，后续可以补充搜索命中率、Rewrite 正确率、首 Token 延迟等指标；
- 服务主要面向受控内网/开发环境，当前版本不建议未经鉴权和限流直接暴露到公网；
- `server.py` 已承载较多兼容逻辑，后续可以进一步拆分 API、搜索编排、缓存和多模态模块。

---

## 10. 后续计划

- [ ] 增加 Search Decision / Query Rewrite 自动化评测集
- [ ] 增加更稳定的引用对齐与来源校验
- [ ] 将模型列表改为从 `llm_hub` 动态同步
- [ ] 抽离缓存层并支持 Redis
- [ ] 补充 API 认证、限流与结构化错误码
- [ ] 拆分 `server.py`，降低单文件复杂度
- [ ] 增加端到端测试和性能 Benchmark
